...
<head>
    ...
    @if (Request::is('home/page/*'))
        @php
            ...
            $single_data = \App\Models\DynamicPage::where('lang_id', session('session_lang_id'))
                ->where('slug', $slug[1])
                ->first();
        @endphp
        <title>{{ $single_data->meta_title }}</title>
    ...
    <style>
        body {
            background-color: #{{ $setting->front_end_color_body }} !important;
        }
        ...
        .bg-area {
            background: #{{ $setting->front_end_color_bg_section }} !important;
            ;
        }

        .footer-area {
            background: #{{ $setting->front_end_color_bg_footer }} !important;
        }
...
<body>
                                    ...
                                    <select name="lang_change_id" class="form-control" onchange="this.form.submit()">
                                        @foreach ($languages as $lang)
                                            <option value="{{ $lang->id }}"
                                                @if ($lang->id == session('session_lang_id')) selected @endif>
                                                {{ $lang->short_name }}</option>
                                        @endforeach
                                    </select>
                                </form>
                            </div>
                                ...
                                <ul>
                                    ...
                                    @if ($count_dynamic_page >= 1)
                                        @foreach ($dynamic_pages as $value)
                                            <li><a
                                                    href="{{ route('home.dynamic.page', $value->slug) }}">{{ $value->name }}</a>
                                            </li>
                                        @endforeach
                                    @endif
                                </ul>
                    ...
                    <div class="searchbar">
                        ...
                        <form method="POST" action="{{ route('home.search') }}">
                            ...
                            <div class="input-group input-search">
                                <input type="text" class="form-control" placeholder="{{ SEARCH_FOR }}"
                                    name="search_string">
    ...
    <!--Call Start-->
    <div class="call-us"
        style="background-image: url({{ asset('upload/' . $footer_setting_lang_independent->cta_background) }}">
        <div class="container">
            <div class="row">
                        ...
                        <form method="POST" action="{{ route('home.subscriber') }}" class="form_subscriber_ajax">
                            ...
                            <div class="input-group">
                                <input type="email" class="form-control" placeholder="{{ EMAIL_ADDRESS }}"
                                    name="email">
                                <span class="input-group-btn">
                                    <button class="btn" type="submit" name="form_subscribe"><i
                        ...
                        <ul>
                            ...
                            @foreach ($all_news as $news)
                                ...
                                @if ($i > $footer_setting_lang_independent->footer_recent_news_item)
                                    @break
                                @endif
                                <li><span><a href="{{ route('news.view', $news->slug) }}"><i
                                                class="fas fa-caret-right"></i> {{ $news->title }}</a></span></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                        ...
                        <div class="row pl-10 pr-10">
                            ...
                            @foreach ($portfolios as $portfolio)
                                ...
                                <div class="col-4 footer-project">
                                    <a href="{{ route('portfolio.view', $portfolio->slug) }}"><img
                                            src="{{ asset('upload/' . $portfolio->photo) }}"
                                            alt="Project Photo"></a>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="col-lg-3 col-md-6">
                    <div class="footer-item">
                        ...
                        <div class="footer-address-item">
                            <div class="icon">
                                @if ($footer_setting->footer_address)
                        ...
                        <div class="footer-address-item">
                            <div class="icon">
                                @if ($footer_setting->footer_phone)
                                    <i class="fa fa-phone"></i>
                                @endif
                            </div>
                            <div class="text">
                        ...
                        <div class="footer-address-item">
                            <div class="icon">
                                @if ($footer_setting->footer_email)
